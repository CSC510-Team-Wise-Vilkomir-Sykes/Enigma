...
class TestEDA(unittest.TestCase):
    ...
    def test_get_longest_charting_songs_empty(self):
        """Test that get_longest_charting_songs returns an empty DataFrame when there are no songs."""
        cursor = self.conn.cursor()
        cursor.execute('DELETE FROM songs')
        self.conn.commit()
        df = get_longest_charting_songs(self.conn)
    ...
    def test_get_longest_charting_songs_no_weeks(self):
        """Test that get_longest_charting_songs returns an empty DataFrame when all songs have 0 weeks on chart."""
        cursor = self.conn.cursor()
