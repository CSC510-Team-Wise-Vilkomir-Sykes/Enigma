...
class TestSongQueueCog(unittest.IsolatedAsyncioTestCase):
	async def asyncSetUp(self):
		...
		await self.bot.add_cog(self.cog)  # Ensure the cog is added to the bot for command registration
	...
	async def test_join_user_not_in_channel(self, mock_log_and_send, mock_is_in_voice_channel):
		...
		mock_log_and_send.assert_called_once_with(mock_ctx,
												  "Please join a voice channel before executing /join")

	@patch('src.bot_state.BotState.is_in_voice_channel', return_value=True)
	@patch('src.bot_state.BotState.log_and_send', new_callable=AsyncMock)
	async def test_join_user_already_in_channel(self, mock_log_and_send, mock_is_in_voice_channel):
		# Mock ctx with user in the same voice channel
		mock_ctx = AsyncMock()
		mock_channel = AsyncMock()
		mock_channel.name = "Test Channel"
		mock_ctx.author.voice.channel = mock_channel
		mock_ctx.guild.voice_client.channel = mock_channel  # Bot is already in the same channel

		await self.cog.join(mock_ctx)

		# Ensure log_and_send was called indicating the bot is already in the channel
		mock_log_and_send.assert_called_once_with(mock_ctx,
												  f"I am already in this voice channel ({mock_channel.name})")

	@patch('src.bot_state.BotState.is_in_voice_channel', return_value=True)
	@patch('src.bot_state.BotState.log_and_send', new_callable=AsyncMock)
	async def test_join_user_switch_channel(self, mock_log_and_send, mock_is_in_voice_channel):
		...
		mock_log_and_send.assert_called_once_with(mock_ctx,
												  f"Switched to voice channel: {mock_user_channel.name}")
	...
	async def test_join_user_channel(self, mock_log_and_send, mock_is_in_voice_channel):
		...
		mock_log_and_send.assert_called_once_with(mock_ctx,
												  f"Joined voice channel: {mock_user_channel.name}")
	...
	async def test_leave_success(self, mock_log_and_send, mock_is_in_voice_channel):
		...
		mock_ctx.guild.voice_client.channel = mock_voice_channel  # Set the channel for the voice client
		...
		# Ensure log_and_send was called with the expected message
		mock_log_and_send.assert_called_once_with(mock_ctx,
												  "Left voice channel: Test Channel")

	@patch('src.bot_state.BotState.is_in_voice_channel', return_value=False)
	@patch('src.bot_state.BotState.log_and_send', new_callable=AsyncMock)
	async def test_leave_not_connected(self, mock_log_and_send, mock_is_in_voice_channel):
		...
		self.assertIsNotNone(command, "Command 'leave' should be registered in the bot.")
	...
	async def test_pause_not_playing(self, mock_log_and_send, mock_is_in_use):
		...
		mock_log_and_send.assert_called_once_with(mock_ctx, "I am currently not playing anything")

	@patch('src.bot_state.BotState.is_in_use', return_value=True)
	@patch('src.bot_state.BotState.is_paused', return_value=True)
	@patch('src.bot_state.BotState.log_and_send', new_callable=AsyncMock)
	async def test_pause_already_paused(self, mock_log_and_send, mock_is_paused, mock_is_in_use):
	...
	@patch('src.bot_state.BotState.log_and_send', new_callable=AsyncMock)
	async def test_pause_success(self, mock_log_and_send, mock_pause, mock_is_paused,
								 mock_is_in_use):
	...
	@patch('src.bot_state.BotState.is_paused', return_value=False)
	@patch('src.bot_state.BotState.log_and_send', new_callable=AsyncMock)
	async def test_unpause_already_unpaused(self, mock_log_and_send, mock_is_paused,
											mock_is_in_use):
	...
	@patch('src.bot_state.BotState.log_and_send', new_callable=AsyncMock)
	async def test_unpause_success(self, mock_log_and_send, mock_unpause, mock_is_paused,
								   mock_is_in_use):
	...
	@patch('src.bot_state.BotState.log_and_send', new_callable=AsyncMock)
	async def test_insert_success(self, mock_log_and_send, mock_insert_song, mock_ensure_song,
								  mock_song_queue):
		# Mock ctx
		mock_ctx = AsyncMock()
		...
		mock_log_and_send.assert_called_once_with(mock_ctx,
												  "Inserted song Test Song as track number 1")

	@patch('src.bot_state.BotState.song_queue', new_callable=list)
	@patch('src.song_queue_cog.SongQueueCog.ensure_song', return_value="Test Song")
	@patch('src.song_queue_cog.SongQueueCog.insert_song', return_value=False)
	@patch('src.bot_state.BotState.log_and_send', new_callable=AsyncMock)
	async def test_insert_failure(self, mock_log_and_send, mock_insert_song, mock_ensure_song,
								  mock_song_queue):
		# Mock ctx
		mock_ctx = AsyncMock()
	...
	@patch('src.song_queue_cog.SongQueueCog.insert_song', return_value=True)
	@patch('src.bot_state.BotState.log_and_send', new_callable=AsyncMock)
	async def test_insertfront_success(self, mock_log_and_send, mock_insert_song, mock_ensure_song,
									   mock_song_queue):
		# Mock ctx
		mock_ctx = AsyncMock()
	...
	@patch('src.bot_state.BotState.log_and_send', new_callable=AsyncMock)
	async def test_jumpto_success(self, mock_log_and_send, mock_ensure_track_number,
								  mock_song_queue):
		# Set up mock context and song queue
		mock_ctx = AsyncMock()
		mock_song_queue.extend(
			["Song 1", "Song 2", "Song 3", "Song 4"])  # Add songs to the queue

		# Invoke the command
		await self.cog.jumpto(mock_ctx, idx="2")  # Jump to track number 2

		# Ensure the song queue is updated correctly
		self.assertEqual(BotState.song_queue, ["Song 2", "Song 3", "Song 4"],
						 "The song queue should start from track 2.")
		mock_log_and_send.assert_called_once_with(mock_ctx,
												  "Jumped to track number 2 in the queue")
	...
	@patch('src.song_queue_cog.SongQueueCog.insert_song', return_value=True)
	@patch('src.bot_state.BotState.log_and_send', new_callable=AsyncMock)
	async def test_move_success(self, mock_log_and_send, mock_insert_song, mock_delete_track,
		...
		mock_log_and_send.assert_called_once_with(mock_ctx,
												  "Moved Song to move from track 1 to track 3")

	@patch('src.song_queue_cog.SongQueueCog.delete_track', return_value="Song to remove")
	@patch('src.bot_state.BotState.log_and_send', new_callable=AsyncMock)
	async def test_remove_success(self, mock_log_and_send, mock_delete_track):
		...
		mock_log_and_send.assert_called_once_with(mock_ctx,
												  "Removed Song to remove (track number 2)")
	...
	async def test_movefront(self, mock_log_and_send, mock_song_queue):
		...
		mock_log_and_send.assert_called_once_with(mock_ctx,
												  "Moved Song 2 from track 2 to track 1")
	...
	async def test_moveback(self, mock_log_and_send, mock_song_queue):
		...
		mock_log_and_send.assert_called_once_with(mock_ctx,
												  "Moved Song 1 from track 1 to track 3")
	...
	async def test_replay_not_playing(self, mock_log_and_send, mock_is_in_use):
		...
		mock_log_and_send.assert_called_once_with(mock_ctx,
												  "I am currently not playing any songs")
	...
	async def test_replay_success(self, mock_insert_song, mock_log_and_send, mock_is_looping,
								  mock_is_in_use, mock_current_song_playing):
		mock_ctx = AsyncMock()
