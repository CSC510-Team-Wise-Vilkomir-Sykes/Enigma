"""
This file is responsible for all bot commands regarding songs such /poll for
...
class SongQueueCog(commands.Cog):
	...
	async def ensure_track_number(ctx, idx):
		"""
		helper function to ensure a valid track number, then convert it from a
        list-start-at-1 idx to a list-start-at-0 idx
	...
	async def ensure_insert_number(ctx, idx):
		"""
		helper function to ensure a valid insertion number, then convert it from a
        list-start-at-1 idx to a list-start-at-0 idx
	...
	async def on_play_query_end(self, ctx, error):
		"""
		function that runs once a song is terminated or ends normally,
        will typically play the next song
		if one is available
	...
	@commands.command(
		name="move",
		help="Move the song at the given track number in a different position in "
             "the queue",
	)
	...
	@commands.command(name="movefront",
                      help="Moves a song to the front of the queue")
	...
	async def volume(self, ctx, volume: int = -1):
		"""
		/volume will adjust the playback volume.
